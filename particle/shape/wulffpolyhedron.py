...
class wulffpolyhedron(polyhedron):
    ...
    n_shapeinfo_111_110_100 = n_vertices_111_110_100 + \
        n_edges_111_110_100 + n_faces_111_110_100

    n_vertices_111_100 = 24
    n_edges_111_100 = 36
    n_faces_111_100 = 14
    n_shapeinfo_111_100 = n_vertices_111_100 + n_edges_111_100 + \
        n_faces_111_100
    ...
    def __init__(self, a, *args, **kwargs):
        ...
        gamma = kwargs.get('gamma', [62.1, 64.1, 67.3])
    ...
    def vertices(self):
        ...
        CoefMat = []
        # (111), (011), (001)
        CoefMat.append(np.array([[1, 1, 1], [0, 1, 1], [0, 0, 1]]))
        # (111), (101), (001)
        CoefMat.append(np.array([[1, 1, 1], [1, 0, 1], [0, 0, 1]]))
        # (111), (101), (100)
        CoefMat.append(np.array([[1, 1, 1], [1, 0, 1], [1, 0, 0]]))
        # (111), (110), (100)
        CoefMat.append(np.array([[1, 1, 1], [1, 1, 0], [1, 0, 0]]))
        # (111), (110), (010)
        CoefMat.append(np.array([[1, 1, 1], [1, 1, 0], [0, 1, 0]]))
        # (111), (011), (010)
        CoefMat.append(np.array([[1, 1, 1], [0, 1, 1], [0, 1, 0]]))
        ...
        for ii in range(6):
            ...
            out[:, ii+6] = mf.EulerRotation(
                np.array([out1[ii][0], out1[ii][1], out1[ii][2]]),
                [0, 0, 90],
                0
            )  # yz-plane
            out[:, ii+12] = mf.EulerRotation(
                np.array([out1[ii][0], out1[ii][1], out1[ii][2]]),
                [0, 0, 180],
                0
            )  # z-axis
            out[:, ii+18] = mf.EulerRotation(
                np.array([out1[ii][0], out1[ii][1], out1[ii][2]]),
                [0, 0, 270],
                0
            )  # zx-plane
            ...
            out[:, ii+24] = np.array([
                out1[ii][0], out1[ii][1], -out1[ii][2]
            ])  # xy-plane
            out[:, ii+30] = mf.EulerRotation(
                np.array([out1[ii][0], out1[ii][1], -out1[ii][2]]),
                [0, 0, 90],
                0
            )   # x-axis
            out[:, ii+36] = mf.EulerRotation(
                np.array([out1[ii][0], out1[ii][1], -out1[ii][2]]),
                [0, 0, 180],
                0
            )  # origin
            out[:, ii+42] = mf.EulerRotation(
                np.array([out1[ii][0], out1[ii][1], -out1[ii][2]]),
                [0, 0, 270],
                0
            )  # y-axis
    ...
    def midpoints(self):
        ...
        for ii in range(8):
            out[:, ii] = out1[:, ii]
            # yz-plane
            out[:, ii+8] = mf.EulerRotation(out1[:, ii], [0, 0, 90], 0)
            # z-axis
            out[:, ii+16] = mf.EulerRotation(out1[:, ii], [0, 0, 180], 0)
            # zx-plane
            out[:, ii+24] = mf.EulerRotation(out1[:, ii], [0, 0, 270], 0)
            # xy-plane
            out[:, ii+32] = np.array([out1[0, ii], out1[1, ii], -out1[2, ii]])
            out[:, ii+40] = mf.EulerRotation(
                np.array([out1[0, ii], out1[1, ii], -out1[2, ii]]),
                [0, 0, 90],
                0
            )   # x-axis
            out[:, ii+48] = mf.EulerRotation(
                np.array([out1[0, ii], out1[1, ii], -out1[2, ii]]),
                [0, 0, 180],
                0
            )  # origin
            out[:, ii+56] = mf.EulerRotation(
                np.array([out1[0, ii], out1[1, ii], -out1[2, ii]]),
                [0, 0, 270],
                0
            )  # y-axis
