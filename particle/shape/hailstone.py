...
class hailstone_with_sphere(object):
    ...
    def __init__(self, shape, *args, **kwargs):
        ...
        '''
        warnings.warn(
            "This class is deprecated. Please use `particle.ensemble_system`",
            DeprecationWarning
        )
        ...
        self.__superpos = False if kwargs.get(
            'superpos') is None else kwargs.get('superpos')

        if self.__superpos is False:
            # Get information from kwargs
            n_hail = 0 if kwargs.get(
                'n_hail') is None else kwargs.get('n_hail')
            ...
            if a_hail_max is None:
                self.a_hail_max = self.__shape.a / 10.
            else:
                self.a_hail_max = a_hail_max
            ...
            if shape.shape_name() in ['sphere', 'spheroid']:
                ...
                self.__center_hails[:, 0] = self.__shape.ax * \
                    np.sin(self.theta)*np.sin(self.phi) + \
                    self.__shape.center[0]
                self.__center_hails[:, 1] = self.__shape.ay * \
                    np.sin(self.theta)*np.cos(self.phi) + \
                    self.__shape.center[1]
                self.__center_hails[:, 2] = self.__shape.az * \
                    np.cos(self.theta) + self.__shape.center[2]
    ...
    def hailsInfo(self):
        _hailsInfo = dict(
            n_hail=self.__n_hails,
            a_hail_max=self.a_hail_max,
            a_hails=self.a_hails,
            center_hails=self.__center_hails
        )
        return _hailsInfo
    ...
    def GetCenterHails(self, angle=False):
        if angle is False:
            ...
            theta = np.arctan2(
                np.sqrt(buff[:, 0]**2+buff[:, 1]**2), buff[:, 2])
            return theta, phi

    def EulerRot(self, euler):
        self.__shape.EulerRot(euler)
        self.center_hails = mf.EulerRotation(
            self.__center_hails, euler, 1) + 0.0
...
class hailstone(object):
    ...
    def info(self):
        ...
        return dict(
            ...
            mother_kwargs=_mother_kwargs,
            daughter_kwargs=_daughter_kwargs
        )
    ...
    def GetCenterOfDaughters(self, angle=False, original=False):
        """GetCenterOfDaughters(self, angle=False, original=False)

        Get the centers of each daughter.
        < Input parameters >
            angle : True = return daughters' positions as polar coordinates
                    (r, theta, phi)
    ...
    def Slice(self, xx, yy, z, *args, **kwargs):
        ...
        # Slice of daughters
        ind_hit = np.where(
            abs(z - self.center_daughters[:, 2]) <= self.a_range_daughters)[0]
        for jj in ind_hit:
