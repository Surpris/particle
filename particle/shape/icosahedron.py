...
class icosahedron(polyhedron):
    ...
    def __init__(self, a, *args, **kwargs):
        ...
        self.ki2 = np.array([
            [phi-1, 0, phi], [phi-1, 0, -phi], [phi, phi-1, 0],
            [-phi, phi-1, 0], [0, phi, phi-1], [0, -phi, phi-1]
        ])/np.linalg.norm(np.array([phi-1, 0, phi]))
    ...
    def vertices(self):
        ...
        vert = np.zeros((3, 12))
        vert[0, :] = a/coef * \
            np.array([phi, 0, -phi, 0, 1, -1, -1, 1, phi, 0, -phi, 0])
        vert[1, :] = a/coef * \
            np.array([0, 1, 0, -1, phi, phi, -phi, -phi, 0, 1, 0, -1])
        vert[2, :] = a/coef * \
            np.array([1, phi, 1, phi, 0, 0, 0, 0, -1, -phi, -1, -phi])
        return vert
    ...
    def midpoints(self):
        ...
        for ii in range(2):
            out[:, count] = np.array([
                (x_med[2*ii] + x_med[2*ii+1])/2,
                (y_med[2*ii] + y_med[2*ii+1])/2,
                (z_med[2*ii] + z_med[2*ii+1])/2
            ])
            count = count + 1
