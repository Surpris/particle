...
def isOutsideSphereIndex(coor, r):
    ...
    <Output>
        The indices of coordinates outside the sphere with
        the radius of `r`
    """
    if coor.shape[1] == 3:
        buff = coor.transpose()
...
def isInsideSphereShellIndex(coor, r_inner, r_outer):
    ...
    <Output>
        The indices of coordinates inside the spherical shell
        defined by (R-dr, R]
    """
...
def isInsideCubeIndex(coor, a):
    """isInsideCubeIndex(coor, a) -> numpy.ndarray

    return indices of coordinates inside a cube with the edge length of `a`.
    ...
    <Output>
        The indices of coordinates inside the cube with
